...
func next_day():
        ...
        # FIX: Inflation only applies from year 2 onwards (not on game start Jan 1, 1970)
        if date["month"] == 1 and date["day"] == 1 and date["year"] > 1970:
                # Jahreswechsel
                var year = date["year"]
...
func process_day_simulation():
        ...
        # Ölpreis Simulation - Enhanced with realistic market dynamics
        _simulate_oil_market()

func _simulate_oil_market():
        # Base volatility depends on era
        var volatility = 0.03  # 1970s: stable pre-crisis
        if date["year"] >= 1973 and date["year"] < 1980:
                volatility = 0.08  # Oil crisis era: high volatility
        elif date["year"] >= 1980 and date["year"] < 1990:
                volatility = 0.05  # 1980s: moderate
        elif date["year"] >= 1990:
                volatility = 0.04  # 1990s+: more stable
        
        # Random market movement
        var daily_change = randf_range(-volatility, volatility)
        
        # Trend momentum - trends tend to continue
        oil_price_trend += daily_change * 0.3
        
        # Mean reversion - prices tend to return to average
        var base_price = 15.0 * inflation_rate  # Inflation-adjusted base
        if date["year"] >= 1973: base_price = 25.0 * inflation_rate
        if date["year"] >= 1979: base_price = 35.0 * inflation_rate
        if date["year"] >= 1986: base_price = 20.0 * inflation_rate  # Oil glut
        if date["year"] >= 1990: base_price = 25.0 * inflation_rate
        
        var reversion_force = (base_price - oil_price) * 0.002
        oil_price_trend += reversion_force
        
        # Apply trend with damping
        oil_price_trend = clamp(oil_price_trend, -0.15, 0.15)
        oil_price += oil_price_trend
        
        # Apply price multiplier (from events)
        var target = oil_price * price_multiplier * tech_bonus_oil_price
        oil_price = lerp(oil_price, target, 0.03)
        
        # Seasonal variation (higher in winter)
        if date["month"] in [11, 12, 1, 2]:
                oil_price *= 1.0 + randf_range(0.0, 0.01)
        elif date["month"] in [5, 6, 7, 8]:
                oil_price *= 1.0 - randf_range(0.0, 0.01)
        
        # Historical price floors/ceilings by era
        var min_price = 3.0
        var max_price = 40.0
        if date["year"] >= 1973: min_price = 8.0; max_price = 60.0
        if date["year"] >= 1979: min_price = 15.0; max_price = 100.0
        if date["year"] >= 1986: min_price = 10.0; max_price = 50.0
        if date["year"] >= 1990: min_price = 15.0; max_price = 80.0
...
func record_history():
        ...
        # FIX: Properly calculate and record revenue and expenses for the month
        var total_revenue = 0.0
        var total_expenses = 0.0
        
        for region_key in current_month_finance:
                var data = current_month_finance[region_key]
                if data.has("revenue"):
                        for cat in data["revenue"]:
                                total_revenue += data["revenue"][cat]
                if data.has("expenses"):
                        for cat in data["expenses"]:
                                total_expenses += data["expenses"][cat]
        
        history_revenue.append(total_revenue)
        history_expenses.append(total_expenses)
        history_profit.append(total_revenue - total_expenses) 
        
func finish_research():
        var completed_tech_id = current_research_id
        researched_techs.append(completed_tech_id)
        current_research_id = ""
        tech_researched.emit(completed_tech_id)
        notify_update()
...
func save_game(slot_name: String = "1"):
        ...
        var save_data = {
                "version": SAVE_VERSION,  # Version for migration compatibility
                "player": {
                        "name": player_name,
                ...
                "economy": {
                        ...
                        "oil_price_trend": oil_price_trend,  # NEW: Save market trend
...
func load_game(slot_name: String = "1"):
        ...
        if save_version < SAVE_VERSION:
                print("Migrating save from version " + str(save_version) + " to " + str(SAVE_VERSION))
        ...
        if data.has("regions"): 
                ...
                for r_name in regions:
                        if loaded_regions.has(r_name):
                                ...
                                # MIGRATION V1->V2: Ensure block_timer exists
                                if not regions[r_name].has("block_timer"):
                                        regions[r_name]["block_timer"] = 0
                                # MIGRATION V2->V3: Ensure offshore_ratio exists
                                if not regions[r_name].has("offshore_ratio"):
                                        regions[r_name]["offshore_ratio"] = 0.0
        
        if data.has("tanks"):
                tank_capacity = data.tanks.get("capacity", {})
                oil_stored = data.tanks.get("stored", {})
                tank_build_year = data.tanks.get("build_year", {})
                tank_investment = data.tanks.get("investment", {})
                # Ensure all regions have tank data
                for r in regions:
                        if not tank_capacity.has(r): tank_capacity[r] = 0
                        if not oil_stored.has(r): oil_stored[r] = 0.0
                        if not tank_build_year.has(r): tank_build_year[r] = 0
                        if not tank_investment.has(r): tank_investment[r] = 0
                
        if data.has("tech"):
                tech_level = data.tech.get("level", 1)
                current_era = data.tech.get("era", 0)
                researched_techs = data.tech.get("researched", [])
                unlocked_techs = data.tech.get("unlocked", [])
                current_research_id = data.tech.get("current_research", "")
                current_research_days_left = data.tech.get("days_left", 0)
                
        if data.has("facilities"): facilities = data.facilities
        
        if data.has("economy"):
                oil_price = data.economy.get("oil_price", 8.50)
                inflation_rate = data.economy.get("inflation", 1.0)
                oil_price_trend = data.economy.get("oil_price_trend", 0.0)  # V3: Load trend
                if contracts_manager:
                        contracts_manager.active_supply_contracts = data.economy.get("contracts", [])
                        contracts_manager.active_futures = data.economy.get("futures", [])
        ...
        if data.has("stats"):
                history_cash = data.stats.get("history_cash", [])
                history_profit = data.stats.get("history_profit", [])
                # V3: Load full history
...
# --- COMPATIBILITY: buy_region for legacy map_screen.gd ---
func buy_region(region_name: String) -> bool:
        # This function is for the old map_screen.gd compatibility
        # It tries to buy a license for the region
        if not regions.has(region_name): return false
        return try_buy_license(region_name)
