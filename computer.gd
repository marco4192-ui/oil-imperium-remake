...
func create_offer_accordion_item(parent, offer, index, is_future):
                                ...
                                _add_detail_row(details_vbox, "Preis:", "$" + GameManager.format_cash(offer["price_per_bbl"]) + " / bbl")
                                ...
                                _add_detail_row(details_vbox, "Gesamtwert:", "$" + GameManager.format_cash(total_val), COL_TERM_MAIN)
                                _add_detail_row(details_vbox, "Strafe (bei Ausfall):", "$" + GameManager.format_cash(offer["penalty"]), COL_TERM_MAIN)
...
func update_stats_view():
                                ...
                                lbl_total_profit.text = "ERGEBNIS (MONAT): " + sign_str + "$" + GameManager.format_cash(profit)
...
func add_stat_row(parent, label, value, color, is_bold=false):
                                ...
                                var v = Label.new()
                                v.text = "$" + GameManager.format_cash(value)
                                v.modulate = color
...
func update_money_display():
                                if lbl_money: lbl_money.text = "$ " + GameManager.format_cash(GameManager.cash, true)
...
func open_facility_details(fac_id):
                                ...
                                if fac["built"]:
                                                                ...
                                                                cost_label.text = "KOSTEN: $" + GameManager.format_cash(int(fac["cost"] * GameManager.inflation_rate)) + "\n" + fac["desc"]
...
func _setup_tech_button(btn, tech_id, tech, era_multiplier, era_locked, _show_icons):
                                ...
                                if tech_id in GameManager.unlocked_techs:
                                                                ...
                                                                btn.text = "KAUFEN\n$" + GameManager.format_cash(hardware_cost)
                                                                ...
                                                                if req_met and GameManager.date["year"] >= tech["year"]:
                                                                                                btn.text = "FORSCHEN\n$" + GameManager.format_cash(research_cost)
...
func update_tank_view(r):
                                ...
                                if tank_status_label2:
                                                                if is_unlocked:
                                                                                                tank_status_label2.text = "REGION: %s\nKAPAZITÄT: %s bbl\nBESTAND: %s bbl" % [r, GameManager.format_cash(cap), GameManager.format_cash(stored)]
...
func _show_loan_menu():
                                ...
                                debt_label.text = " | Gesamtschulden: $" + GameManager.format_cash(GameManager.loan_manager.get_total_debt())
                                ...
                                for offer in offers:
                                                                ...
                                                                offer_info.text = "%s: $%s @ %.1f%% (%d Monate)" % [
                                                                                                offer["name"], GameManager.format_cash(offer["principal"]), 
