...
func update_view():
                ...
                if region_locked:
                                ...
                                action_btn.text = "LIZENZ KAUFEN ($" + GameManager.format_cash(cost_k) + "k)"
...
func update_info_panel(claim: Dictionary):
                ...
                if region_locked:
                                info += "STATUS: GESPERRT\nPREIS: $" + GameManager.format_cash(price)
                                ...
                                if ai_owner != null:
                                                ...
                                                if not claim.get("surveyed", false):
                                                                if is_instance_valid(btn_secondary):
                                                                                ...
                                                                                btn_secondary.text = "EXPERTISE ($" + GameManager.format_cash(s_cost) + ")"
                                                                info += "DATEN: UNBEKANNT\n\n[EXPERTISE ERFORDERLICH]"
                                                else:
                                                                # Survey was done - show quality indicator
                                                                ...
                                                                info += "SICHERHEIT: " + str(confidence) + "%\n\n"
                                                                info += "PROGNOSE: ~" + str(yield_val) + " BBL/TAG\n"
                                                                info += "VOLUMEN: ~" + str(res_val) + "k BBL\n"
                                                ...
                                                if is_instance_valid(btn_secondary): 
                                                                btn_secondary.visible = true
                                                                btn_secondary.text = "VERKAUFEN ($" + GameManager.format_cash(s_val) + ")"
                                else:
                                                ...
                                                if claim.get("has_oil", false):
                                                                ...
                                                                # Show how accurate the survey was
                                                                var survey_yield = claim.get("survey_yield", 0)
                                                                var actual_yield = claim.get("yield", 0)
                                                                if survey_yield > 0 and actual_yield > 0:
                                                                                var accuracy = 100 - abs(survey_yield - actual_yield) / max(survey_yield, actual_yield) * 100
                                                                                info += "\n(Expertise war " + str(int(accuracy)) + "% genau)"
                                                else: 
                                                                info += "ERGEBNIS: TROCKEN\n"
                                                                if claim.get("surveyed", false):
                                                                                info += "(Expertise war UNGENAU)"
                                                action_btn.text = "-"
                                                action_btn.disabled = true
...
func start_drilling_process():
                ...
                lbl_total_self.text = "SUMME: $" + GameManager.format_cash(cost_self)
                if is_instance_valid(lbl_time_self): lbl_time_self.text = "Dauer: " + str(duration_self) + " Tage"
                lbl_total_expert.text = "SUMME: $" + GameManager.format_cash(cost_expert)
                ...
                lbl_cost_list.text = "Logistik: $" + GameManager.format_cash(c_log) + "\nMiete: $" + GameManager.format_cash(c_rent) + "\nCrew: $" + GameManager.format_cash(c_crew)
